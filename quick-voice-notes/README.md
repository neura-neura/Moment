...
For global capture from a game or another full-screen app, use [Quick Capture Bridge (WinUI 3)](../quick-capture-bridge-winui/README.md). Import this plugin's settings in the bridge; it then records WebM, runs local Whisper, and routes Markdown/audio output directly to the vault without requiring this plugin. This plugin remains available for the in-Obsidian command path. The public download link is pending publication.
