...
public sealed class NativeDailyNoteService
{
    ...
    public NativeDailyNoteService(BridgeSettings settings) => this.settings = settings;
    ...
    public string WriteCapture(string text, DateTimeOffset timestamp)
    {
        ...
        if (string.IsNullOrWhiteSpace(settings.VaultPath) || !Directory.Exists(settings.VaultPath))
            throw new InvalidOperationException("Choose an existing Obsidian vault before writing a Daily Note.");
        ...
        lock (gate)
        {
            ...
            var heading = settings.IncludeTimestamp
                ? MomentFormat.Format(local, string.IsNullOrWhiteSpace(settings.DailyTimestampFormat) ? "HH:mm" : settings.DailyTimestampFormat)
                : "";
    ...
    public DailyNoteProviderSettings ReadProviderSettings() => DailyNoteProviderSettings.Load(settings.VaultPath);
...
public sealed record DailyNoteProviderSettings(string Folder, string Format, string Template)
{
    public static DailyNoteProviderSettings Load(string vault)
    {
        var periodic = Path.Combine(vault, ".obsidian", "plugins", "periodic-notes", "data.json");
        if (Read(periodic, out var periodicDoc) && GetObject(periodicDoc, "daily", out var daily) && GetBool(daily, "enabled", out var enabled) && enabled)
            return From(daily);
        var core = Path.Combine(vault, ".obsidian", "daily-notes.json");
        if (Read(core, out var coreDoc)) return From(coreDoc);
        var legacy = Path.Combine(vault, ".obsidian", "plugins", "daily-notes", "data.json");
        if (Read(legacy, out var legacyDoc)) return From(legacyDoc);
        return new DailyNoteProviderSettings("", "YYYY-MM-DD", "");
    }

    private static DailyNoteProviderSettings From(JsonElement value)
    {
        var format = GetString(value, "format");
        return new DailyNoteProviderSettings(GetString(value, "folder"), string.IsNullOrWhiteSpace(format) ? "YYYY-MM-DD" : format, GetString(value, "template"));
    }
...
internal static class VaultPath
{
    public static string Combine(string folder, string name)
    {
        ...
        if (safeName.Length == 0 || safeName.Split('/').Any(p => p is "" or "..")) throw new InvalidOperationException("The Daily Note filename is invalid.");
    ...
    public static string Resolve(string root, string relative)
    {
        ...
        if (!full.StartsWith(basePath, StringComparison.OrdinalIgnoreCase)) throw new InvalidOperationException("The vault path escapes the selected vault.");
        return full;
    }
...
internal static class TemplateRenderer
{
    ...
    public static string Load(string vault, string templatePath, string filename, DateTimeOffset capture, string providerFormat)
    {
        if (string.IsNullOrWhiteSpace(templatePath) || templatePath.Trim() == "/") return "";
        try
        {
            var path = VaultPath.Resolve(vault, templatePath.Trim());
...
internal static class NativeCaptureInsertion
{
    ...
    public static string Insert(string current, string heading, string text, BridgeSettings settings)
    {
        var block = string.IsNullOrWhiteSpace(heading) ? text.Trim() : $"## {heading}\n\n{text.Trim()}";
        return settings.DailyInsertionLocation switch
        {
            "beginning" => InsertAt(current, FrontmatterEnd(current), block),
            "under-heading" => UnderHeading(current, block, settings.DailyTargetHeading, settings.DailyMissingHeadingBehavior),
