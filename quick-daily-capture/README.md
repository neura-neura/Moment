...
For global capture while Obsidian is closed, use [Quick Capture Bridge (WinUI 3)](../quick-capture-bridge-winui/README.md). It accepts any key combination (including F13-F24), shows a compact PiP panel, and writes directly to the selected vault. Import this plugin's settings in the bridge, then this plugin can be disabled or uninstalled; it remains available for users who prefer the in-Obsidian command. The public download link is pending publication.
