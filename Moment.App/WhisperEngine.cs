...
public sealed class NativeWhisperEngine
{
    ...
    public NativeWhisperEngine()
    {
        ...
        TryMigrateLegacyDirectory(Path.Combine(localAppData, "QuickCaptureBridge", "whisper"), momentDirectory);
        dataDirectory = momentDirectory;
    }

    public async Task<WhisperEngineStatus> GetStatusAsync(MomentSettings settings, CancellationToken cancellationToken = default)
    {
    ...
    public async Task InstallAsync(MomentSettings settings, Action<WhisperInstallProgress>? progress = null, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<string> TranscribeAsync(string audioPath, MomentSettings settings, CancellationToken cancellationToken = default)
    {
        ...
        var work = Path.Combine(Path.GetTempPath(), $"moment-whisper-{Guid.NewGuid():N}");
    ...
    private static HttpClient CreateHttpClient() => new(new HttpClientHandler { AllowAutoRedirect = true })
    {
        ...
        DefaultRequestHeaders = { { "User-Agent", "Moment/1.6.1" } }
    };

    private static void TryDelete(string path) { try { if (File.Exists(path)) File.Delete(path); } catch { } }

    private static void TryMigrateLegacyDirectory(string legacy, string target)
    {
        if (Directory.Exists(target) || !Directory.Exists(legacy)) return;
        try
        {
        ...
        {
            // A locked legacy cache can remain in place; the user can use
            // Install / repair Whisper to create the new Moment cache.
        }
    }
}
