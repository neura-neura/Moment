...
public sealed class CaptureController : IDisposable
{
    ...
    public CaptureController(MomentSettings initialSettings)
    {
        settings = initialSettings;
        inbox = new CaptureStore(settings);
    ...
    public void Register(MomentSettings next)
    {
        Unregister();
        settings = next;
        inbox = new CaptureStore(settings);
    ...
    public void ShowVoicePiP()
    {
        ...
        {
            SetStatus("Choose an existing workspace folder before recording.", false);
            return;
        }
        ...
        try
        {
            ...
            panel.Saved += _ =>
            {
                SetStatus("Voice note saved; processing in the background.", true);
    ...
    public void ShowTextPiP()
    {
        ...
        if (!inbox.IsConfigured)
        {
            SetStatus("Choose an existing workspace folder before writing a note.", false);
            return;
        }
...
{
    private readonly CaptureStore inbox;
    ...
    public TextPiP(CaptureStore inbox, MomentSettings settings)
    {
        this.inbox = inbox;
    ...
    private void InputKeyDown(object sender, KeyRoutedEventArgs args)
    {
        ...
        if (args.Key == VirtualKey.Enter)
        {
            ...
            if (!shift && settings.RecurringNoteEnterToSave)
            {
                args.Handled = true;
                Save();
            }
    ...
    private void Save()
    {
        ...
        try
        {
            ...
            if (settings.RecurringNoteCloseAfterSave)
            {
                HideForClose();
                CloseSoon(0);
            }
...
public sealed class VoicePiP : Window
{
    ...
    private readonly Border surface;
    private readonly StackPanel waves;
    private readonly StackPanel noSignalText;
    ...
    public VoicePiP(CaptureStore inbox, MomentSettings settings)
    {
        this.inbox = inbox;
        this.settings = settings;
        Title = "";
        ...
        AutomationProperties.SetName(noSignalText, "No audio. Check the selected microphone.");
        ...
        var recordingGrid = new Grid
        {
            ...
            Children = { recordingIndicator, waves, noSignalText, actions }
        };
        Grid.SetColumn(waves, 1);
        Grid.SetColumn(noSignalText, 1);
    ...
    private void StartRecording()
    {
        try
        {
            ...
            waves.Visibility = Visibility.Visible;
    ...
    private void ShowNoSignalWarning(bool final)
    {
        ...
        waves.Visibility = Visibility.Collapsed;
        AutomationProperties.SetName(noSignalText, final
            ? "No audio detected. Recording discarded. Check the selected microphone."
            : "No audio detected. Check the selected microphone.");
        var hwnd = WindowNative.GetWindowHandle(this);
        NativeWindowUtilities.ConfigureOverlay(hwnd, 260, 60, OverlayPlacement.CenterTop, false);
    }
