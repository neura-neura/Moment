...
The production installer is published in the [Moment v1.2.3 release](https://github.com/neura-neura/Moment/releases/tag/v1.2.3). Local development builds produce a real NSIS Windows installer EXE through `npm run package:bridge`; it has a setup wizard and uninstaller, and no MSIX certificate is required. Moment can stay hidden in the Windows notification tray when its settings window is closed, and its two capture panels are compact topmost overlays rather than normal application windows.
